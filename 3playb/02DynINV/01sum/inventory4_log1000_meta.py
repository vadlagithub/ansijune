...
'''
# *Section1* USER: as SCRIPT: ./inventory.py :time ansible all -i inventory.py --list-hosts ***
# *Section2* Enabling LOGGER: Proceed Section1: and: cat /var/tmp/ansible_dynamic_inventory.log******
   # To clr log: cat /devel/null > /var/tmp/ansible_dynamic_inventory.log ***************************
   # ./inventory2_log --host centos1 and: cat log and clear log *************************************
# *Section3* If 1000 hosts: ansible all -i inv --list-hosts-> hangs so cancel it and tail -f LOG &***
   # rerun timed --list-hosts :jobs : cat LOG and: kill %1 :bgrnd LOG & clear LOG  ******************
# *Section4* Meta: Proceed Section1: ./inventory.py --list | more: do timed --list-hosts> instant o/p
